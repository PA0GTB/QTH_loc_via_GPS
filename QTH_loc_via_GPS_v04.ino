...
------------------------------------------------------------
Original design found in VERON Electron, November 2015 by Hans Wagemakers, ON4CDU (all credits !)
Modified for showing coordinates on LCD by ON4CDU for PE1PIC
Modified by PA0GTB for typical I2C display, structured the code and add comments  ( May 2017)
If wanted: Connect external LED via PIN 13 via 470 Ohm to ground
*/
...
const int pinled = 13;  //connection for external led
const int EOled = 8; 
...
// Characters for QTH locator String
...
void setup()
{
  ...
  lcd.setCursor(0,2); lcd.print (" Wacht op GPS Data! ");
  lcd.setCursor(0,3); lcd.print ("     - PA0GTB -    ");
...
void printGGAinfo ()
{
  ...
  {
    alternate = seconds / 15;
    
    switch (alternate)
    {
      case 0:
      {
        sprintf (sz, "! %2d Sats" , gps_sats);
      ...
      case 1:
      {
        sprintf (sz, "!  HDOP %1d"); // Aangepast Pe1PIC ==
        break;
      }
      case 2:
      {
        // Show coordinates
        sprintf (sz, "!   %3dm " , alti); // Aangepast Pe1PIC ==
        lcd.setCursor(0,2); lcd.print ("Lat:  ");lcd.print (latf,5);lcd.print((char)223); lcd.print ("  "); lcd.print (latdir);lcd.print ("   ");
        lcd.setCursor(0,3); lcd.print ("Lon:   ");lcd.print (lonf,5);lcd.print((char)223); lcd.print ("  "); lcd.print (longdir); 
        break;
      }
      case 3:
      {
        sprintf (sz, "! %3d/%3d" , azd,eld); // Aangepast Pe1PIC ==
        break;
      }
    }
  }
  ...
  {
    // Show number of Satelites
    alternate = (seconds % 2);
    if (0 == alternate)
    {
      sprintf (sz, "? %2d Sats" , gps_sats); // Aangepast Pe1PIC ==
    }
    else
    {
      sprintf (sz, "  %2d Sats" , gps_sats); // Aangepast Pe1PIC ==
...
void locator() // locator calculation and display
{
  ...
  lcd.setCursor(0, 1);lcd.print ("Loc:"); // printing QTH locator on screen 
  ...
  // lcd.print(Lastchar_string[int(scrap * 24)]);
